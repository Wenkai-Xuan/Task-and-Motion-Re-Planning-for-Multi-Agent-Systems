...
with open('a2_traj_5.json', 'r') as file:
    states2 = json.load(file)
joint_states2 = []
for i in range(298):   # 474 joint states in total
    joint_states2.append(states2[i]['joint_state'])

with open('a3_traj_5.json', 'r') as file:
    states3 = json.load(file)
joint_states3 = []
for i in range(298):
    joint_states3.append(states3[i]['joint_state'])
...
obj_pos = []
obj_quat = []
for i in range(298):   # 474 object states in total
    obj_pos.append([a - b for a, b in zip(obj[i]['pos'], [0, 0, 0.45])]) # join_states:(474,7), obj[i]['pos'] - (0, 0, 0.6) as the table's height 
...
obj_quat1 = []
for i in range(298):
    obj_pos1.append([a - b for a, b in zip(obj1[i]['pos'], [0, 0, 0.45])])
...
while not glfw.window_should_close(window):
    ...
    data.qpos[18:24] = joint_states3[current_step][0:6]

    data.qpos[24:27] = obj_pos[current_step][0:3]
    data.qpos[27:31] = obj_quat[current_step][0:4]
    data.qpos[31:34] = obj_pos1[current_step][0:3]
    data.qpos[34:38] = obj_quat1[current_step][0:4]
