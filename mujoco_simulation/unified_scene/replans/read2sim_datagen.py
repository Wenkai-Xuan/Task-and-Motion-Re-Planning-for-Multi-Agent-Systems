...
#     json_type = convert_numpy(df[text][sample_indx]) #unnecessary
#     with open(f'{origin_folder}'+ '/' +f'{text}.json', 'w') as file:
#         json.dump(json_type, file, indent=4)
...
for i in range(len(part_hold_dict)):
    ...
    while current_step < max_steps:
        ...
        if not diff(old_plan[-1], old_plan[-2], 1e-5): #quit the for-loop when objects stay statically
            break
