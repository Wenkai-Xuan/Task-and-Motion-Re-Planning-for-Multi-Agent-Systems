...
# load the joint states from json file
with open('a0_traj_5.json', 'r') as file:
    states = json.load(file)
joint_states = []
for i in range(438):   # 474 joint states in total
    joint_states.append(states[i]['joint_state']) # join_states:(474,7)

with open('a1_traj_5.json', 'r') as file:
...
with open('obj1_traj_5.json', 'r') as file:
    obj = json.load(file)
obj_pos = []
obj_quat = []
for i in range(438):   # 474 object states in total
    obj_pos.append([a - b for a, b in zip(obj[i]['pos'], [0, 0.05, 0.55])]) # join_states:(474,7), obj[i]['pos'] - (0, 0, 0.6) as the table's height 
    obj_quat.append(obj[i]['quat'])

with open('obj2_traj_5.json', 'r') as file:
    obj1 = json.load(file)
obj_pos1 = []
obj_quat1 = []
for i in range(438):
    obj_pos1.append([a - b for a, b in zip(obj1[i]['pos'], [0, 0.05, 0.55])])
    obj_quat1.append(obj1[i]['quat'])

with open('obj3_traj_5.json', 'r') as file:
    obj2 = json.load(file)
obj_pos2 = []
obj_quat2 = []
for i in range(438):   
    obj_pos2.append([a - b for a, b in zip(obj2[i]['pos'], [0, 0.05, 0.55])])
    obj_quat2.append(obj2[i]['quat'])

with open('obj4_traj_5.json', 'r') as file:
    obj3 = json.load(file)
obj_pos3 = []
obj_quat3 = []
for i in range(438):   
    obj_pos3.append([a - b for a, b in zip(obj3[i]['pos'], [0, 0.05, 0.55])])
    obj_quat3.append(obj3[i]['quat'])
...
while not glfw.window_should_close(window):
    ...
    data.qpos[12:15] = obj_pos[current_step][0:3]
    data.qpos[15:19] = obj_quat[current_step][0:4]
    data.qpos[19:22] = obj_pos1[current_step][0:3]
    data.qpos[22:26] = obj_quat1[current_step][0:4]
    data.qpos[26:29] = obj_pos2[current_step][0:3]
    data.qpos[29:33] = obj_quat2[current_step][0:4]
    data.qpos[33:36] = obj_pos3[current_step][0:3]
    data.qpos[36:40] = obj_quat3[current_step][0:4]
